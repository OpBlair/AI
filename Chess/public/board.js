...
//Square colors
const lightSquare = 'bg-yellow-50'; //soft cream
const darkSquare = 'bg-amber-700'; //warm chocolate
...
function isPathClear(fromId, toId, dCol, dRow) {
...
//Pawn Rule
function isValidPawnMove(fromId, toId, pieceCode) {
    ...
    //straight move
    if (targetPiece)
        return false; // cannot move straight into occupied square
    //one square move
    if (rowDiff === 1)
        return true;
...
function isValidRookMove(fromId, toId) {
    const from = getCoords(fromId);
    const to = getCoords(toId);
...
function isValidBishopMove(fromId, toId) {
    const from = getCoords(fromId);
    const to = getCoords(toId);
    const dCol = to.col - from.col;
    const dRow = to.row - from.row;
...
function isValidQueenMove(fromId, toId) {
...
function isValidKnightMove(fromId, toId) {
    const from = getCoords(fromId);
    const to = getCoords(toId);
...
function isValidKingMove(fromId, toId) {
    const from = getCoords(fromId);
    const to = getCoords(toId);
    const dCol = Math.abs(to.col - from.col);
    const dRow = Math.abs(to.row - from.row);
...
// === Master Validation ===
function isValidMove(fromId, toId, pieceCode) {
    const targetPiece = boardState[toId];
...
function renderPiece(square, pieceCode) {
...
function handleSquareClick(event) {
    ...
    const square = clickedElement.closest('[data-row]');
    if (!square)
        return;
